...
class BaseCapturer:
    def __init__(self):
        self.cached_device = None  # 💡 高速化のためデバイス情報をキャッシュ

    def get_current_device(self, force_check=False):
...
class MacCapturer(BaseCapturer):
    ...
    def initialize_devices(self):
        self.get_current_device(force_check=True)

    def get_current_device(self, force_check=False):
        # 💡 キャッシュがあればそれを一瞬で返す（通信オーバーヘッドをゼロにする）
        if self.cached_device and not force_check:
    ...
    def capture(self, output_path):
        ...
        if not device:
            # デバイスが見つからない場合は1度だけ再チェック
            device = self.get_current_device(force_check=True)
        ...
        if device == "ios":
            ...
            if res.returncode != 0:
                self.cached_device = None  # エラー時はキャッシュクリア
                return None
            return "iOS"
        elif device == "android":
    ...
    def _init_windows_firewall(self):
        try:
            if os.path.exists(self.adb):
                subprocess.run([self.adb, "start-server"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False, creationflags=CREATE_NO_WINDOW)
                subprocess.run([self.adb, "devices"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False, creationflags=CREATE_NO_WINDOW)
            if os.path.exists(self.ios):
                subprocess.run([self.ios, "list"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False, creationflags=CREATE_NO_WINDOW)
        except:
            pass
...
def get_capturer():
    return WindowsCapturer() if platform.system() == "Windows" else MacCapturer()
...
def auto_capture_loop():
    ...
    while is_running:
        ...
        # コマンド実行に時間がかかった場合は、sleepを挟まずに即座に次へ
...
def download_selected():
    data = request.json
    filenames = data.get('filenames', [])
    zip_name = data.get('zipName', 'captures')
...
def rename_image():
    data = request.json
    old_display = data.get('oldDisplay', '')
    new_display = data.get('newDisplay', '')
